...
public class SenderClient {
  ...
  // 日志
  private static Logger logger = Logger.getLogger(SenderClient.class);
   ...
   * @throws Exception
   */
  public static void startSenderClient() {
    try {
      ...
      bootstrap
          ...
          .handler(new SenderClientInitializer()); // 添加编码解码和处理器
    } catch (Exception e) {
  ...
  public static Channel getConnection(int port) {
    return bootstrap.connect(host, port).addListener(new ChannelFutureListener() {
      ...
      public void operationComplete(ChannelFuture future) throws Exception {
         ...
         */
        if (future.isSuccess()) {
          logger.info(host + " " + port + "连接smtp成功");
        } else {
          logger.debug(host + " " + port + "连接smtp失败");
//          // 连接不成功，5秒后重新连接
...
//          }, 5, TimeUnit.SECONDS);
  ...
  public static void shutDownSenderClient() {
    if ( group != null ) {
